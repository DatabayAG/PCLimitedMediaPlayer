...
/**
 * GUI class to show the player and handle updates
 * This GUI is shown in an iframe
 *
...
class ilPCLimitedMediaPlayerGUI
{
    ...
    public function __construct()
    {
        ...
        $this->parent_id = $this->get->integer('parent_id', 0);
        $this->page_id = $this->get->integer('page_id', 0);
        $this->file_id = $this->get->string('file_id', '');
        $this->preview_id = $this->get->string('preview_id');
        $this->height = $this->get->integer('height');
        $this->width = $this->get->integer('width');
        $this->limit_context = LimitContext::tryFrom((string) $this->get->string('limit_context'))
            ?? LimitContext::from(LimitContext::TESTPASS);
        $this->limit_plays = $this->get->integer('limit_plays');
        $this->play_with_pause = $this->get->bool('play_with_pause', false);
    ...
    private function showPlayer()
    {
        ...
        $this->current_plays = $this->usage->getPlays();
        $this->current_seconds = $this->usage->getSeconds();
        ...
        $tpl = $this->plugin->getTemplate("tpl.iframe_player.html");
    ...
    private function updateUsage()
    {
        ...
        $this->usage->setProgress($plays ?? 0, $seconds);
        ...
        $this->http->saveResponse($this->http->response()->withBody(Streams::ofString(json_encode([
                'status' => $this->usage->getStatus($this->limit_plays, false)->value(),
                'seconds' => $this->usage->getSeconds(),
                'plays' => $this->usage->getPlays()
            ]))));
        $this->http->sendResponse();
        $this->http->close();
    }
